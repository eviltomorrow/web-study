...
对象的比较：是引用的比较，当且仅当它们引用同一个基对象时，它们才相等。

## 3.8 类型转换

如果 JavaScript 期望使用一个布尔值，它把给定的值将转换为布尔值。

如果 JavaScript 期望使用一个字符串，它把给定的值将转换为字符串。

如果 JavaScript 期望使用一个数字，它把给定的值将转换为数字（如果转换结果无意义将返回 NaN）